                                            ...
                                            const String& deviceIdentifierToOpen)
{
    ...
    while (--numPorts >= 0)
    {
                ...
                & (forInput ? SND_SEQ_PORT_CAP_SUBS_READ : SND_SEQ_PORT_CAP_SUBS_WRITE)) != 0)
        {
            ...
            if (clientName != "jackmidi" && clientName != "jack_midi" && clientName != "a2jmidid")
            {
                ...
                if (deviceIdentifierToOpen.isNotEmpty() && deviceIdentifierToOpen == device.identifier)
                {
                    if (portID != -1)
                    {
                        String name (forInput ? JUCE_ALSA_MIDI_INPUT_NAME : JUCE_ALSA_MIDI_OUTPUT_NAME);
                        name += "_" + String (deviceIdentifierToOpen);
                        port = client->createPort (name, forInput, false);
